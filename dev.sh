...
GREEN='\033[00;32m'
YELLOW='\e[0;33m'
HOST_PROD=twitterclone.example.com
...
workon twitterclone  # Change this to your project's name
...
function dknpminstall {
    CD=$(pwd)
    cd $PROJ_BASE
    docker run -it --rm -v $(pwd):/app -w /app/frontend -e NODE_ENV=development twitterclone npm install
    exitcode=$?
    cd $CD
    return $exitcode
}
...
function dkrun_prod {
    docker stop twitterclone
    docker rm twitterclone
    docker run --name twitterclone -d --env-file /home/ubuntu/twitterclone.env \
        -p 3000:3000 -p 8000:8000 \
        -v /home/ubuntu/dkdata/twitterclone:/dkdata \
        twitterclone start_web.sh
}
...
echo_green "Welcome to twitterclone's dev env"
